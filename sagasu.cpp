...
void row_migi(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
	int i;
    for (i = 2; i < 8 - y; i++) {
...
void row_hidari(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
    int i;
	for (i = 2; i < y+1; i++) {
        if (row[x][y - i] == b && search_zahyou(basyo,x,y)==false) {
        	basyo.push_back({x,y});
...
void clm_ue(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
    int i;
	for (i = 2; i < 8 - x; i++) {
...
void clm_sita(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
	int i;
    for (i = 2; i < x+1; i++) {
        if (row[x-i][y] == b && search_zahyou(basyo,x,y)==false) {
        	basyo.push_back({x,y});
...
void miginaname_ue(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
	int i;
...
void miginaname_sita(vector<vector<int>>& row, int x, int y, int b, vector<vector<int>>& basyo) {
    int i;
...
void hidarinaname_ue(vector<vector<int>>& row,int x,int y,int b,vector<vector<int>>& basyo){
	int i;
...
void hidarinaname_sita(vector<vector<int>>& row,int x,int y,int b,vector<vector<int>>& basyo){
	int i;
